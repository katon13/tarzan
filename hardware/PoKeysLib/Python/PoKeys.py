...
class sPoKeysPEv2(Structure):
    _fields_ = [
        ...
        ("reserved_align", c_uint8*4),         

        ("EncoderIndexCount", c_uint32),
        ("EncoderTicksPerRotation", c_uint32),
        ("EncoderVelocity", c_uint32),            
        ("reservedEnd", c_uint32),            

        ("InternalDriverStepConfig", c_uint8*4),         
        ("InternalDriverCurrentConfig", c_uint8*4),         
    ]
...
class sPoILStatus(Structure):
        ...
        ("ExceptionLocation", c_uint32),
        ("reserved_for_64", c_uint32),
        ("ExceptionDescription", c_uint32*4),
...
class sPoKeysNetworkDeviceInfo(Structure):
    _fields_ = [
        ...
        ("AdditionalNetworkOptions", c_uint8),         # Additional network options: bits 7:4 are 0xA, lower are the following:
                                                       # - bit 3: reserved
...
class PoKeysDevice:
    ...
    def PK_PoStep_ConfigurationSet(self):
        return self.libObj.PK_PoStep_ConfigurationSet(self.device)
    def PK_PoStep_ConfigurationGet(self):
        return self.libObj.PK_PoStep_ConfigurationGet(self.device)
    def PK_PoStep_StatusGet(self):
        return self.libObj.PK_PoStep_StatusGet(self.device)
    def PK_PoStep_DriverConfigurationGet(self):
        return self.libObj.PK_PoStep_DriverConfigurationGet(self.device)
    def PK_PoStep_DriverConfigurationSet(self):
        return self.libObj.PK_PoStep_DriverConfigurationSet(self.device)

    def PK_PEv2_InternalDriversConfigurationGet(self):
        return self.libObj.PK_PEv2_InternalDriversConfigurationGet(self.device)
    def PK_PEv2_InternalDriversConfigurationSet(self):
        return self.libObj.PK_PEv2_InternalDriversConfigurationSet(self.device)
    ...
    def PK_CANConfigure(self, bitrate):
        return self.libOjb.PK_CANConfigure(self.device, bitrate)

    def PK_CANRegisterFilter(self, format, CANid):
        return self.libOjb.PK_CANRegisterFilter(self.device, format, CANid)

    def PK_CANWrite(self, msg : sPoKeysCANmsg):        
        return self.libObj.PK_CANWrite(self.device, msg)

    def PK_CANRead(self, msg : sPoKeysCANmsg, status):
        readMsg = sPoKeysCANmsg()
        status = c_uint8(0)

        func = self.libObj.PK_CANRead
        func.argtypes = [sPoKeysDevicePtr, POINTER(sPoKeysCANmsg), POINTER(c_uint8)]
        result = func(self.device, readMsg, status)
        if result != 0:
            return None
    ...
    def PK_CANFlush(self):
        return self.libOjb.PK_CANFlush(self.device)
