...
class TarzanTakePreviewWindow(tk.Toplevel):
    ...
    def refresh(
        ...
        self.header_var.set(
            ...
            f"preview_pulses={total_generated_pulses} | full_cycle_pulses={int(getattr(axis_take, 'full_cycle_pulses', 0))} | "
    ...
    def _resolve_target_pulses(self, axis_take, validation_result: Any | None) -> int:
        ...
        if validation_result is not None:
            resolved = float(getattr(validation_result, "pulses_total", 0.0) or 0.0)

        if resolved <= 0:
            generated_protocol = getattr(axis_take, "generated_protocol", {}) or {}
            raw_signal = getattr(axis_take, "raw_signal", {}) or {}

            for key in ("step_count_total",):
                value = generated_protocol.get(key)
        ...
        if resolved <= 0:
            ...
            if segments:
                resolved = sum(float(getattr(seg, "pulse_count", 0) or 0) for seg in segments)

        if full_cycle > 0:
            resolved = min(resolved, full_cycle)

        return max(0, int(round(resolved)))
